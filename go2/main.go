...
func (api *API) NewTokenHandler(ctx *fasthttp.RequestCtx) {
	...
	var result []NewToken
	if err := api.tntConn.ExecTyped(tarantool.Call("get_new_token", []interface{}{}), &result); err != nil {
		log.Println(err)
...
func (api *API) RevokeTokenHandler(ctx *fasthttp.RequestCtx) {
	...
	if err := api.tntConn.ExecTyped(tarantool.Call("revoke_token", []string{tokenUUID}), &result); err != nil {
		ctx.SetStatusCode(500)
		_, _ = fmt.Fprintf(ctx, "%s", err)
		return
	}
...
func (api *API) CheckTokenHandler(ctx *fasthttp.RequestCtx) {
	tokenUUID := string(ctx.QueryArgs().Peek("token"))
	var result []bool
	if err := api.tntConn.ExecTyped(tarantool.Call("check_token", []string{tokenUUID}), &result); err != nil {
		ctx.SetStatusCode(500)
		_, _ = fmt.Fprintf(ctx, "%s", err)
		return
	}
...
func main() {
	conn, err := tarantool.Connect("tcp://localhost:3301", tarantool.Opts{
		Concurrency: 32,
		User:        "tokens",
		Password:    "tokens",
	})
